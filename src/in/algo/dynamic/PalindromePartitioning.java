...
public class PalindromePartitioning {
	...
	// Similar to Matrix Multiplication Problem, Try to partition for every
	// possible index.
	public static int palindromePartitionRecur(String str, int index1, int index2) {
		...
		if (isPalidrome(str.substring(index1, index2 + 1)))
			return 0;
		// Take a hihger Value,
		// Partition the problem into subproblems by keeping a partition at
		// every place. Recursively solve them,
		// Get the results, use them to construct the mininum of partitions for
		// this substring.
		int val = Integer.MAX_VALUE;
		for (int i = index1; i < index2; i++) {
			// Recursively get the values for left substring and right substring
			// and add ONE ( one is current partition ).
			val = Math.min(val,
					palindromePartitionRecur(str, index1, i) + palindromePartitionRecur(str, i + 1, index2) + 1);
		}
		return val;
	}
	...
	public static int palindromPartition_DP(String str) {
		...
		for (int i = 0; i < str.length(); i++) {
			// If the subString is Palindrome. Then the cuts[i] ->
			// subStirng[0..i] will be 0; Period
			// If not, check from subString[1..i], subString[2..i] so on uptill
			// subString[i..i]
			if (isPalindrome[0][i])
				cuts[i] = 0;
			else {
				...
				for (int j = 0; j < i; j++) {
					...
					// at last right part is palindrome for [i][i] So, cuts[i] =
					// cuts[j]+1;
